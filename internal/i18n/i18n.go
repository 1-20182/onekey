...
var translations = map[string]map[string]string{
	"zh": {
		...
		"task.steampp_conflict": "检测到 {tool} 正在运行，它与 Onekey 的解锁内核冲突，会导致游戏“解锁成功但库不显示”。请先完全退出 {tool} 后再解锁。",
		// SteamTools
	...
	"en": {
		...
		"task.steampp_conflict":        "Detected {tool} is running. It conflicts with Onekey's unlock kernel and games may 'unlock OK but not appear'. Please fully quit {tool} first.",
		...
		"error.steamtools_setup":       "SteamTools config write failed: {error}",
		"error.kernel_download_failed": "Kernel download failed",
		// Steam actions
